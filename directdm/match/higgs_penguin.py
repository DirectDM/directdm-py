...
class Higgspenguin(object):
    ...
    def hisano_fbc(self, mchi):
        ...
        return np.real(3*self.alpha**2/self.sw**4 * ((self.dchi**2 - (4*Y**2+1))/(8*self.MW**3) * self.__gW(w,y) + Y**2/(4*self.MZ**3*self.cw**4) * self.__gZ(z,y)))
    ...
    def oneloop_ew(self, mchi):
        ...
        def gH(x):
            ...
            return out
        return (self.alpha)**2/(4*self.Mh**2*self.sw**4) * ((self.dchi**2 - (4*Y**2+1))/(8*self.MW) * gH(w) + Y**2/(4*self.MZ*self.cw**4) * gH(z))

    def oneloop_light(self, mchi):
        return -3*(self.alpha)**2/(8*self.MW**2*self.Mh**2*self.sw**4*self.cw**2) * mchi * \
                  ( self.Ychi**2 + self.cw**2 * ((self.dchi**2 - 1) - self.Ychi**2) )
